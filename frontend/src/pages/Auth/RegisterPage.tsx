...
const RegisterPage = () => {
  ...
  const fullName = useMemo(() => `${data.firstName} ${data.lastName}`.trim(), [data.firstName, data.lastName])
  ...
  const handleFirstNameChange = useCallback((value: string) => setData((p) => ({ ...p, firstName: value })), [])
  const handleLastNameChange = useCallback((value: string) => setData((p) => ({ ...p, lastName: value })), [])
  const handleEmailChange = useCallback((value: string) => setData((p) => ({ ...p, email: value })), [])
  const handlePasswordChange = useCallback((value: string) => setData((p) => ({ ...p, password: value })), [])
  const handleConfirmPasswordChange = useCallback((value: string) => setData((p) => ({ ...p, confirmPassword: value })), [])
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (hasFieldErrors) {
      const firstError =
        fieldValidation.firstName || fieldValidation.lastName ||
        fieldValidation.email || fieldValidation.password ||
        fieldValidation.confirmPassword || 'Please fix the highlighted fields.'
    ...
    setIsLoading(true)
    try {
      const result = await authService.register({ fullName, email: data.email, password: data.password, role: 'user' })
      ...
      notifications.show({ title: 'Account created', message: 'Registration successful', color: 'green' })
      navigate('/dashboard')
    } catch (err: any) {
      if (!isMountedRef.current) return
  ...
  }, [data.password, data.confirmPassword, data.email, data.firstName, data.lastName, fullName, navigate])
              ...
              >
                <div className="rounded-2xl border border-surface-container bg-surface-container-low p-4 text-center sm:text-left shadow-sm">
                  ...
                  <div className="mt-3 flex items-center gap-2 justify-center sm:justify-start">
                    <h3 className="text-base font-bold text-on-surface">Smart Curation</h3>
                    <Badge text="AI" variant="primary" className="text-[10px] px-2 py-0.5" />
                  </div>
                ...
                <div className="rounded-2xl border border-surface-container bg-surface-container-low p-4 text-center sm:text-left shadow-sm">
                  ...
                  <div className="mt-3 flex items-center gap-2 justify-center sm:justify-start">
                    <h3 className="text-base font-bold text-on-surface">Agent Logic</h3>
                    <Badge text="New" variant="success" className="text-[10px] px-2 py-0.5" />
                  </div>
                  <p className="mt-1 text-xs text-slate-500">
